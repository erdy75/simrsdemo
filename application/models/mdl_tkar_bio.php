...
class Mdl_tkar_bio extends CI_Model {
  ...
  {
    $data = $this->db->get_where('tkar_bio',array('id'=>$id));
    if($data->num_rows()==1){
  ...
  function count_daftar_karyawan($cari_nama,$order_p)
  {
    $text = '';
    if($cari_nama!='')
    {
  ...
  function get_all()
  {
    ...
    $query = $this->db->get('tkar_bio');
    return $query->result_array();
  }
  
  public function fetch_daftar_karyawan($limit, $start,$cari_nama,$order_p) 
  {
    $text = '';
    if($cari_nama!='')
    {
  ...
  public function show_data_by_id($id)
  {
    $query = $this->db->query('SELECT 
      ...
      a.sex,
      a.darah,
      DATE_FORMAT(a.TMT_masuk,"%d-%m-%Y") as TMT_masuk ,
