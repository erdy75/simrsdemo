...
class M_gol_darah extends CI_Model {
  ...
  {
    $data = $this->db->get_where('gol_darah',array('id'=>$id));
    if($data->num_rows()==1){
  ...
  function count_filter_where($filter) 
  {
    ...
    if($filter!='')
    {
      $this->db->like('gol_darah', $filter, 'both');
    } 
    $query=$this->db->get($table);
    $num_rows = $query->num_rows();
    return $num_rows;
  ...
  public function fetch_gol_darah($limit, $start,$filter) 
  {
    $this->db->select('id, gol_darah');
