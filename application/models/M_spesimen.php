...
class M_spesimen extends CI_Model {
  ...
  {
    $data = $this->db->get_where('spesimen',array('id'=>$id));
    if($data->num_rows()==1){
  ...
  function get_all()
  {
    $query = $this->db->get('spesimen');
    return $query->result_array();
  }


  public function fetch_spesimen($limit, $start,$filter,$bidang_id) 
  {
    $this->db->select('id, spesimen');
    $this->db->from('spesimen');
    $this->db->where('bidang', $bidang_id);
    $this->db->limit($limit, $start);
    if($filter!='')
    {
      $this->db->like('spesimen', $filter, 'both');
    } 
    $this->db->order_by('id','asc');
