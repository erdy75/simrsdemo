...
class Mdl_lab_default_kelompok_tarif extends CI_Model {
  ...
  function get_table() 
  {
    $table = 'lab_default_kelompok_tarif_list';
    return $table;
  }
  ...
  {
    $data = $this->db->get_where('lab_bidang',array('nama'=>$id));
    if($data->num_rows()==1){
  ...
  function _update($perawatan, $data)
  {
    $table = $this->get_table();
    $this->db->where('perawatan', $perawatan);
  ...
  function get_all()
  {
    $query = $this->db->get('lab_default_kelompok_tarif_list');
    return $query->result_array();
  }
