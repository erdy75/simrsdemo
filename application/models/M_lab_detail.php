...
class M_lab_detail extends CI_Model {
  ...
  {
    $data = $this->db->get_where('lab_detail',array('id'=>$id));
    if($data->num_rows()==1){
  ...
  public function fetch_lab_detail($limit, $start,$filter,$bidang_id) 
  {
    $this->db->select('id, bidang, layanan, id_bidang, persiapan, unitcost, metode');
    $this->db->from('v_lab_detail');
    $this->db->where('id_bidang', $bidang_id);
