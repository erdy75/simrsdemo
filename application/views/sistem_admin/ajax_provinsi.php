...
$('a#batal').click(function() {
  ...
  $.ajax({
    type: "POST",
    data: "negara_id=0",
    url: "<?=base_url()?>index.php/sistem/provinsi/show_negara_by_id/",
    success: function(msg) {
      $("#td_negara_id").html(msg);
    }
  });
  $("input[name='id']").val('');
  $("button#simpan").show();
...
$("a#<?=$list_data[$i]['id']?>").click(function() {
  $("input[name='nama']").val("<?=$list_data[$i]['nama_prov']?>");
  $.ajax({
    type: "POST",
    data: "negara_id=<?=$list_data[$i]['negara_id']?>",
    url: "<?=base_url()?>index.php/sistem/provinsi/show_negara_by_id/",
    success: function(msg) {
      $("#td_prov_id").html(msg);
    }
